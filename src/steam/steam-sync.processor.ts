...
import { PlatformConnection, SyncStatus } from '../platform-connections/platform-connection.entity';
...
export class SteamSyncProcessor extends WorkerHost {
  ...
  async process(job: Job<SteamSyncJobData>): Promise<void> {
    ...
    await this.connectionsRepository.update(connectionId, { syncStatus: SyncStatus.SYNCING });

    try {
      const ownedGames = await this.steamApiService.getOwnedGames(steamId64);

      for (const owned of ownedGames) {
        const coverUrl = SteamApiService.coverUrl(owned.appid);
        const game = await this.gamesService.upsertBySteamAppId(owned.appid, owned.name, coverUrl);
        const lastPlayedAt =
      ...
      const syncError = ownedGames.length === 0 ? 'No games found — the Steam profile may be private.' : null;
      ...
      this.logger.error(`Steam sync failed for connection ${connectionId}`, error);
      await this.connectionsRepository.update(connectionId, {
