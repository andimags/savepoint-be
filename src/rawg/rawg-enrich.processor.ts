...
export class RawgEnrichProcessor extends WorkerHost {
  ...
  async process(_job: Job): Promise<void> {
    if (!this.rawgApiService.isConfigured) {
      this.logger.warn('Skipping RAWG enrichment — RAWG_API_KEY not configured');
      return;
    }
    ...
    for (const game of games) {
      try {
        const results: RawgGame[] = await this.rawgApiService.searchGames(
          toSearchQuery(game.name),
          5,
        );
        const match = results.find((r) => isPlausibleMatch(game.name, r.name));
        if (match) {
