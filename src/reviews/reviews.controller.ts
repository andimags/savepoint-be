...
export class ReviewsController {
  ...
  forGame(
    ...
    return this.reviewsService.findForGame(gameId, req.user.userId, clampPage(page), clampLimit(limit));
  }

  @Get('reviews/recent')
  recent(@Request() req: Req, @Query('page') page?: string, @Query('limit') limit?: string) {
    return this.reviewsService.findRecent(req.user.userId, clampPage(page), clampLimit(limit));
  }
  ...
  byUser(
    ...
    return this.reviewsService.findByUser(userId, req.user.userId, clampPage(page), clampLimit(limit));
  }
  ...
  comments(
    ...
    return this.reviewsService.findComments(id, clampPage(page), clampLimit(limit));
  }

  @HttpCode(HttpStatus.NO_CONTENT)
  @Delete('comments/:id')
  async removeComment(@Request() req: Req, @Param('id', ParseUUIDPipe) id: string) {
