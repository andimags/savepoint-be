...
export class ReviewsService {
    ...
    async create(
        ...
        return this.reviewsRepository.save(
            this.reviewsRepository.create({ userId, gameId, body }),
        );
    }
    ...
    async like(userId: string, reviewId: string): Promise<void> {
        ...
        if (!existing) {
            await this.likesRepository.save(
                this.likesRepository.create({ userId, reviewId }),
            );
        }
    }
    ...
    async addComment(
        ...
        return this.commentsRepository.save(
            this.commentsRepository.create({ userId, reviewId, body }),
        );
    }
    ...
    private async assertReviewExists(reviewId: string): Promise<void> {
        const exists = await this.reviewsRepository.exists({
            where: { id: reviewId },
        });
