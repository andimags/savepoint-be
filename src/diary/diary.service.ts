...
export class DiaryService {
  ...
  async update(userId: string, entryId: string, input: Partial<DiaryEntryInput>): Promise<DiaryEntry> {
  ...
  private async getOwnEntry(userId: string, entryId: string): Promise<DiaryEntry> {
    const entry = await this.diaryRepository.findOne({ where: { id: entryId } });
    if (!entry) throw new NotFoundException('Diary entry not found');
    if (entry.userId !== userId) throw new ForbiddenException('Not your diary entry');
    return entry;
  }
}
