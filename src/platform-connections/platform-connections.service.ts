...
import { PlatformConnection, Platform, SyncStatus } from './platform-connection.entity';
...
export class PlatformConnectionsService {
  ...
  async connectSteam(userId: string, profileUrlOrId: string): Promise<PlatformConnection> {
    const steamId64 = await this.steamApiService.resolveToSteamId64(profileUrlOrId);
  ...
  async resync(userId: string): Promise<PlatformConnection> {
    ...
    if (!connection || !connection.steamId64) {
      throw new BadRequestException('No Steam connection found for this user');
    }
    connection.syncStatus = SyncStatus.PENDING;
    connection.syncError = null;
  ...
  getStatus(userId: string): Promise<PlatformConnection | null> {
    return this.connectionsRepository.findOne({ where: { userId, platform: Platform.STEAM } });
  }
