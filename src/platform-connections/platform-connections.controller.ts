...
export class PlatformConnectionsController {
  ...
  @Post('steam')
  connectSteam(@Request() req: { user: { userId: string } }, @Body() dto: ConnectSteamDto) {
    return this.platformConnectionsService.connectSteam(req.user.userId, dto.profileUrlOrId);
  }
  ...
  async status(@Request() req: { user: { userId: string } }) {
    const connection = await this.platformConnectionsService.getStatus(req.user.userId);
    if (!connection) {
