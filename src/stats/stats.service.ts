...
export class StatsService {
    ...
    async overview(userId: string) {
        const totals = await this.userGamesRepository
            ...
            .addSelect(
                "COALESCE(SUM(ug.playtimeMinutes), 0)",
                "totalPlaytimeMinutes",
            )
            .addSelect(
                `COUNT(*) FILTER (WHERE ug.status = '${GameStatus.FINISHED}')`,
                "finished",
            )
            .addSelect(
                `COUNT(*) FILTER (WHERE ug.status = '${GameStatus.PLAYING}')`,
                "playing",
            )
            .addSelect(
                `COUNT(*) FILTER (WHERE ug.status = '${GameStatus.BACKLOG}')`,
                "backlog",
            )
            .addSelect(
                `COUNT(*) FILTER (WHERE ug.status = '${GameStatus.DROPPED}')`,
                "dropped",
            )
       ...
       LIMIT 12`,
        ...
        return {
            ...
            completionRate:
                started > 0 ? Number((finished / started).toFixed(3)) : null,
    ...
    async wrapped(userId: string, year: number, month?: number) {
        ...
        const diaryEntries = await this.diaryRepository
            ...
            .andWhere(
                "entry.playedOn >= :fromStr AND entry.playedOn < :toStr",
                {
                    fromStr,
                    toStr,
                },
        ...
        const playedByLastPlayed = await this.userGamesRepository
            .createQueryBuilder("ug")
            .where("ug.userId = :userId", { userId })
            .andWhere("ug.lastPlayedAt >= :from AND ug.lastPlayedAt < :to", {
                from,
                to,
            })
            .getMany();
        ...
        const totalMinutes = rows.reduce(
            (sum, row) => sum + row.playtimeMinutes,
            0,
        );
