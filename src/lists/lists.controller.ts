...
export class ListsController {
  ...
  create(@Request() req: Req, @Body() dto: ListDto) {
    return this.listsService.create(req.user.userId, dto.title, dto.description ?? null);
  }
  ...
  @Patch(':id')
  update(@Request() req: Req, @Param('id', ParseUUIDPipe) id: string, @Body() dto: ListDto) {
    return this.listsService.update(req.user.userId, id, dto.title, dto.description ?? null);
  }
  ...
  @Post(':id/items')
  addItem(@Request() req: Req, @Param('id', ParseUUIDPipe) id: string, @Body() dto: AddItemDto) {
