...
export class RecommendationsService {
    ...
    async forUser(userId: string) {
        ...
        const owned = await this.userGamesRepository.find({
            where: { userId },
        });
        const ownedIds = new Set(owned.map((ug) => ug.gameId));

        let candidates = await this.scoreLocalCandidates(
            genreWeights,
            ownedIds,
        );
        ...
        if (
            ...
            try {
                const rawgGames =
                    await this.rawgApiService.getGamesByGenres(topGenres);
                for (const rawgGame of rawgGames) {
                    await this.gamesService.upsertFromRawg(rawgGame);
                }
                candidates = await this.scoreLocalCandidates(
                    genreWeights,
                    ownedIds,
                );
            } catch {
    ...
    private async scoreLocalCandidates(
