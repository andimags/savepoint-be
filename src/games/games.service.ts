...
export class GamesService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (pending > 0) {
      this.logger.log(`Queuing RAWG enrichment for ${pending} unenriched games`);
  ...
  async upsertBySteamAppId(steamAppId: number, name: string, coverUrl: string | null): Promise<Game> {
    const existing = await this.gamesRepository.findOne({ where: { steamAppId } });
    if (existing) {
      existing.name = name;
    ...
    const game = this.gamesRepository.create({ steamAppId, name, coverUrl });
    return this.gamesRepository.save(game);
  }

  async upsertFromRawg(rawgGame: RawgGame): Promise<Game> {
    let game = await this.gamesRepository.findOne({ where: { rawgId: rawgGame.id } });
    if (!game) {
  ...
  async getById(id: string): Promise<Game> {
    ...
    if (game.rawgId && !game.description && this.rawgApiService.isConfigured) {
      try {
  ...
  async markEnrichmentAttempted(gameId: string): Promise<void> {
    await this.gamesRepository.update(gameId, { rawgEnrichedAt: new Date() });
  }
  ...
  async enrichGame(game: Game, rawgGame: RawgGame): Promise<Game> {
    ...
    const duplicate = await this.gamesRepository.findOne({ where: { rawgId: rawgGame.id } });
    ...
    game.releaseDate = rawgGame.released ?? game.releaseDate;
    game.metacritic = rawgGame.metacritic ?? game.metacritic;
    if (rawgGame.description_raw) game.description = rawgGame.description_raw;
    game.rawgEnrichedAt = new Date();
    return this.gamesRepository.save(game);
  }

  countMissingRawgData(): Promise<number> {
    return this.gamesRepository.count({ where: { rawgId: IsNull(), rawgEnrichedAt: IsNull() } });
  }
