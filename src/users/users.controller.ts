...
export class UsersController {
  ...
  async uploadAvatar(@Request() req: Req, @UploadedFile() file?: UploadedImage) {
    if (!file) {
    ...
    const avatarUrl = await this.cloudinaryService.uploadAvatar(dataUri, req.user.userId);
    const user = await this.usersService.setAvatarUrl(req.user.userId, avatarUrl);
    return toMe(user);
  }
}
